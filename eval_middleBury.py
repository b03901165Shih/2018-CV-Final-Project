...
def evaluate(input_path, gt_path, scale_factor, threshold=1):
...
def main():
    ...
    res = evaluate('./result/tsukuba.png', './testdata/tsukuba/disp3.pgm', scale_factor=16)
    avg += res
    print('Tsukuba: %.2f%%' % (res*100))

    res = evaluate('./result/venus.png', './testdata/venus/disp2.pgm', scale_factor=8)
    avg += res
    print('Venus: %.2f%%' % (res*100))

    res = evaluate('./result/teddy.png', './testdata/teddy/disp2.png', scale_factor=4)
    avg += res
    print('Teddy: %.2f%%' % (res*100))

    res = evaluate('./result/cones.png', './testdata/cones/disp2.png', scale_factor=4)
    avg += res
